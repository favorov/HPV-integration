...
system "bwa aln -l 32 -k 2 -t 4 $fastaFile -b $fastqfname > $sai_name";
...
system "ex_summary.pl $sum_gt3_file";
system "rm -f $whole_sam_name $sai_name $sam_name $final_clean_sam_file $reportFile $bamFile $sBFforIndex $graphCoverageFile $gt4CoverageFile $outGraphFile $summaryCoverage $sum_gt3_file $filtered_sam";
